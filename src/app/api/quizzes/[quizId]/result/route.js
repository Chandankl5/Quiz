...
export async function GET(request, context) {
  ...
  const { params: { quizId } } = context;

  if (!data.quizzes.allIds.includes(quizId)) {
    return NextResponse.json({ message: 'Quiz Not Found' }, { status: '404' })
  }

  try {
    const totalQuestions = data.quizzes.byIds[quizId].questions.allIds.length;

    const correctChoiceCount = Math.floor(Math.random() * (totalQuestions  + 1))
  
    let responseData = {
      score: (correctChoiceCount / totalQuestions) * 100,
      correctChoiceCount,
      incorrectChoiceCount: totalQuestions - correctChoiceCount
    }
